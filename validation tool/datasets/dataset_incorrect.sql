...
Insert into R3 -- références de type de coprocessus
(A1,A2,A3)
values
...
(5), -- déplacement 2 bis (case d'accueil) - echec (e.g pas de case de destination)
(6), -- déplacement 1 (case de destination)
(7), -- déplacement 2 (case de destination)
(8), -- déplacement 3 (case de destination) - reussite
(9); -- déplacement 3 bis (case de destination) - echec (e.g. déjà occupée)

Insert into R5 -- références de type d'opérations de coprocessus
(A1,A2,A3)
values
...
(2,5,0), -- déplacement par case d'accueil - déplacement 2 bis (case d'accueil) - echec
...
(A1,A2)
values
(1,1), -- déplacement 1 par personnage
(2,3), -- déplacement 1 par case d'accueil
(3,6); -- déplacement 1 par case de destination

Insert into R8 -- références d'opérations terminant un coprocessus
(A1,A2)
values
(1,2), -- déplacement 2 par personnage
(2,4), -- déplacement 2 par case d'accueil
(2,5), -- déplacement 2 bis par case d'accueil
(3,8), -- déplacement 3 par case de destination
(3,9); -- déplacement 3 bis par case de destination
...
(4,1,1,2,0,1), -- déplacement reussi / ref 2 - Personnage (4) - coprocessus 1 - deplacement rang 2 -- erreur numero 4 l'opération de type 1 effectuée par le personnage est suivie d'une autre opération de type 1, alors que c'est l'opération de type 2 qui est attendue
...
-- processus 2
(4,2,2,1,0,2), -- déplacement échoué pas de case D / ref 1 - Personnage (4) -- erreur numero 3 l'opération de type 2 n'est pas une opération initiale pour le coprocessus
(4,2,2,2,0,1), -- déplacement échoué pas de case D / ref 2 - Personnage (4)
(2,2,2,1,0,3), -- déplacement échoué pas de case D / ref 3 - case B (2) -- erreur numero 1 : il n'y a qu'une opération dans ce coprocessus
...
(3,3,1,2,0,7); -- déplacement échoué / ref 7 - case C (3) -- erreur numero 6 il manque la troisième opération de ce coprocessus, celle-ci n'étant pas une opération terminale

insert into R15
(A1,A2,A3,A4,A5,A6)
VALUES
(2,1,1,3,1,'inoccupee'); -- erreur numero 2 : la valeur attendue est 'occupee' et non pas 'inoccupee'
